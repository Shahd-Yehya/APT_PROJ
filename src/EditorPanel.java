...
public class EditorPanel extends JFrame {
    ...
    // ─── CRDT & networking ────────────────────────────────────────────────
    private final Document    crdtDoc;
    ...
    public EditorPanel(int siteId) {
        ...
        this.siteId  = siteId;
        this.crdtDoc = new Document(new BlockCRDT());
    ...
    private void buildUI() {
        ...
        boldBtn.addActionListener(e -> applyFormat(true, false));
        italicBtn.addActionListener(e -> applyFormat(false, true));
    ...
    public void refreshTextPane() {
        ...
        updatingText = true;
        try {
            String newText = crdtDoc.getFullText();
            ...
            if (!newText.equals(oldText)) {
                ...
                int safePos = Math.min(caretPos, newText.length());
    ...
    private void applyFormat(boolean bold, boolean italic) {
        ...
        // Identify visible block containing selection (simplified: first block)
        List<BlockNode> blocks = crdtDoc.getVisibleBlocks();
        ...
        for (int i = start; i < end && i < nodes.size(); i++) {
            CharacterId cid = nodes.get(i).getId();
            CRDTOperation op = bn.getContent()
                    .format(cid, bn.getBlockId(), siteId, bold, italic);
            if (client != null && op != null)
                client.sendOperation(op);
        }

        // Apply Swing-side styling too
